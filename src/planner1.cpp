...
---------------------------------------------------------------------*/
bool pathService(local_planner_raph::PathToGoal::Request& request, local_planner_raph::PathToGoal::Response& response)
{
...
int main(int argc, char **argv)
{
	...
	while (ros::ok())
	{
		...
		distCurTarget = sqrt(pow(pathPoses.back().pose.position.x - curPose2D.x, 2) + pow(pathPoses.back().pose.position.y - curPose2D.y, 2) );

		if( distCurTarget < 0.15 )
		{
			if( pathPoses.size() > 1 ) {
				pathPoses.pop_back();
				ROS_INFO("### SIZE  --- x=%.2f   y=%.2f,    size=%d", pathPoses.back().pose.position.x, pathPoses.back().pose.position.y, (int)pathPoses.size());
			}
		}

		distCurTarget = sqrt(pow(pathPoses.back().pose.position.x - curPose2D.x, 2) + pow(pathPoses.back().pose.position.y - curPose2D.y, 2) );

		angleCurTarget = atan2( pathPoses.back().pose.position.y - curPose2D.y , pathPoses.back().pose.position.x - curPose2D.x );

		angle = shortestAngleDiff(angleCurTarget, curPose2D.theta);

		ROS_INFO("### Pose2d: X=[%.1f] %.1f    Y=[%.1f] %.1f    theta=[%.1f] %.1f  ---  dist=%.2f  angle=%.2f", pathPoses.back().pose.position.x,
																												curPose2D.x,
																												pathPoses.back().pose.position.y,
																												curPose2D.y,
																												angleCurTarget,
																												curPose2D.theta,
																												distCurTarget, angle);
		twist.angular.z = std::min(angle * Ka, angSat);

		if(fabs(angle) < 0.02) {
			twist.linear.x = std::min(distCurTarget * Kp, linSat);
		}
